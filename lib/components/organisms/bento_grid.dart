...
class BentoGrid extends StatelessComponent {
  const BentoGrid({required this.blocks, this.layoutType, super.key});
  ...
  Iterable<Component> build(BuildContext context) sync* {
    yield _buildAsymmetricalBentoGrid(blocks, layoutType);
  }
  ...
  Component _buildAsymmetricalBentoGrid(
    ...
    // For showcase layout, use the curated order. For others, sort dynamically.
    final orderedBlocks = layoutType == 'showcase'
        ? blocks
        : _sortBlocksDynamically([...blocks]);

    // Generate truly asymmetrical grid areas based on project count and types
    final gridAreas = _generateBentoAreas(orderedBlocks, layoutType);
    final layoutClass = layoutType == 'showcase'
        ? 'showcase'
        : _getLayoutClass(blocks.length);

    return div(classes: 'bento-grid__container bento-layout--$layoutClass', [
      for (int i = 0; i < orderedBlocks.length && i < gridAreas.length; i++)
        div(
          classes:
              'bento-grid__item ${_getSizeClass(orderedBlocks[i])} bento-area--${gridAreas[i]}',
          [
            if (orderedBlocks[i].type == BentoBlockType.project)
              ProjectCard(
                project: orderedBlocks[i].project!,
                size: ProjectCardSize.fromProject(orderedBlocks[i].project!),
              )
            else
              AccentCard(accent: orderedBlocks[i].accent!),
          ],
        ),
    ]);
  }

  List<BentoBlock> _sortBlocksDynamically(List<BentoBlock> blocks) {
    blocks.sort((a, b) {
  ...
  List<String> _generateBentoAreas(
    List<BentoBlock> blocks,
    String? layoutType,
  ) {
    ...
    return _generateAsymmetricalBentoAreas(blocks);
  }
  ...
  static List<StyleRule> get styles => [
    ...
    css('.bento-grid').styles(
      raw: const {
        ...
        'max-width': '1600px',
        'margin': '0 auto',
        'padding': '0 2rem', // Padding for the grid container itself
      },
    ),
    ...
    css('.bento-grid__container').styles(
      raw: const {
        'display': 'grid',
        'gap': '1.5rem',
        'grid-auto-rows': 'minmax(180px, auto)', // Reduced base row height
      },
    ),

    // Showcase layout - our hand-crafted 2x4 grid
    css('.bento-layout--showcase').styles(
      raw: const {
        ...
        'grid-template-areas': '''
          "top-left top-mid top-mid top-right"
          "bot-left bot-mid bot-mid bot-right"
        ''',
      },
    ),
    ...
    css('.bento-area--top-left').styles(raw: const {'grid-area': 'top-left'}),
    css('.bento-area--top-mid').styles(raw: const {'grid-area': 'top-mid'}),
    css('.bento-area--top-right').styles(raw: const {'grid-area': 'top-right'}),
    css('.bento-area--bot-left').styles(raw: const {'grid-area': 'bot-left'}),
    css('.bento-area--bot-mid').styles(raw: const {'grid-area': 'bot-mid'}),
    css('.bento-area--bot-right').styles(raw: const {'grid-area': 'bot-right'}),
